...
import (
	...
	"github.com/drewbitt/circadian/routes"
...
func ensureCollections(app *pocketbase.PocketBase) {
	...
	if _, err := app.FindCollectionByNameOrId("sleep_records"); err != nil {
		...
		if err := app.Save(c); err != nil {
			log.Printf("Failed to create sleep_records collection: %v", err)
		}
	}
	...
	if _, err := app.FindCollectionByNameOrId("energy_schedules"); err != nil {
		...
		if err := app.Save(c); err != nil {
			log.Printf("Failed to create energy_schedules collection: %v", err)
		}
	}
...
func runMorningJobForAllUsers(app *pocketbase.PocketBase) {
	...
	if err != nil {
		log.Printf("Failed to find users for morning job: %v", err)
		return
	}
	for _, user := range users {
		if err := services.RunMorningJob(app, user.Id); err != nil {
			log.Printf("Morning job failed for user %s: %v", user.Id, err)
		}
	}
}
...
func syncFitbitForAllUsers(app *pocketbase.PocketBase) {
	...
	for _, s := range settings {
		...
		if token.Expiry.Before(time.Now()) {
			...
			if err != nil {
				log.Printf("Fitbit token refresh failed for user %s: %v", userID, err)
				continue
			}
		...
		for _, date := range []time.Time{time.Now().AddDate(0, 0, -1), time.Now()} {
			...
			if err != nil {
				log.Printf("Fitbit sync failed for user %s: %v", userID, err)
				continue
			}
